...
const projectParser = async function(etsProjectFilePath, workdir) {
  ...
  const parse = async (parseDeviceApplicationInformation) => {
    ...
    if ((err = await findProjectInfoFolder())) {
      return err
    }
    if ((err = await parseProjectInformation())) {
      return err
    }
    if ((err = await parseProject())) {
      return err
    }
    if ((err = await new ProductInformationParser(self).parseHardwareInformationFiles())) {
      return err
    }
    if ((err = await new KnxMasterXMLParser(self).parseKNXMasterFile())) {
      return err
    }
