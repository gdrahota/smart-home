  ...
  <v-card>
    <v-card-text>
      <v-layout wrap row class="subheading pb-2 pt-2">
        ...
        <v-flex xs2 pr-2>Befehl</v-flex>
        <v-flex xs3 pr-3>An-Wert</v-flex>
        <v-flex xs3 pr-3>Aus-Wert</v-flex>
        <v-flex xs1></v-flex>
      </v-layout>
      ...
      <v-layout v-for="(command, idx) of schedule.commands" :key="'c' + idx" wrap row command>
        ...
        <v-flex xs2 pr-2><span>{{ getCommandEndpoint(command) }}</span></v-flex>
        <v-flex xs3 pr-3>
          <SetValue
            :endpoint="getEndpoint(command)"
            :value="command.maxValue"
            defaultValueName="max"
            @setValue="v => { command.maxValue = v }"
          />
        </v-flex>
        <v-flex xs3 pr-3>
          <SetValue
            :endpoint="getEndpoint(command)"
            :value="command.minValue"
            defaultValueName="min"
            @setValue="v => { command.minValue = v }"
          />
        </v-flex>
        <v-flex xs1>
          <confirm
...
<script>
  ...
  import AddCommand from './add-command-time-slots'
